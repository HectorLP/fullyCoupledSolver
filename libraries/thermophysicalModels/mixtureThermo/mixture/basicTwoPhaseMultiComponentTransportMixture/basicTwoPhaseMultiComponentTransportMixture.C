...
:
    ...
    cYi_
    (
        readScalar(alpha1.mesh().solutionDict().subDict("PISO").lookup("cYi"))
    ),
...
surfaceScalarField Foam::basicTwoPhaseMultiComponentTransportMixture::compressionCoeff(const label i)
{
    ...
    surfaceScalarField deltaYi2 = YiDown/(alphaDown+(1-alphaDown)*HY_[i])-HY_[i]*YiUp/(alphaUp+(1-alphaUp)*HY_[i]);

    return sign(deltaYi2)*max(mag(deltaYi1),mag(deltaYi2));
