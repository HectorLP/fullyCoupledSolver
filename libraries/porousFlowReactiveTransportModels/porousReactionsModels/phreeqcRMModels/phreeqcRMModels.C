...
:
    basicPorousReactionsModels(mesh, dict, Yf),
    phreeqcDict_(dict.subDict(typeName)),
    transportPropertiesDict_(dict),
    activateUpdatePorosity_(phreeqcDict_.lookup("activateUpdatePorosity")),
    setComponentH2O_
...
void Foam::porousReactionsModels::phreeqcRMModels::initializeFluidCompositionNew()
{
    const std::vector<std::string> & components = phreeqc_.GetComponents();
    int tmpNumComponent = components.size();
    ...
    Info << "nspecies = " << tmpNumComponent << endl;
    for (int i = 0; i < tmpNumComponent; ++i)
    {
        Info << "The solution component is " << components[i] << endl;
    }
    int nspecies = phreeqc_.GetSpeciesCount();
    ...
    // }
    // componentSolutionIndex_ = (int*)malloc((size_t)(nspecies * sizeof(int)));
    // forAll(speciesNames, i)
    // {
    //     for (j = 0; j < nspecies; ++j)
    //     {
    //         std::tring tmpSpecies = speciesNames[j];
    //         if tmpSpcies == speciesNames[i]
    //         {
    //             componentSolutionIndex_p[i] = j;
    //         }
    //     }
    // }
    // if (nspecies > 0) concentration_ = (double*)malloc((size_t)(nspecies * nxyz_ * sizeof(double)));
    // status = phreeqc_.GetSpeciesConcentrations(concentration_);
    // forAll(speciesNames, i)
    // {
    //     volScalarField& Yi = Yf_[i];
    //     forAll(mesh_.cells(), celli)
    //     {
    //         concentration_[componentSolutionIndex_[i] * nxyz_ + celli] = Yi[celli] / 1000.;
    //     }
    // }

    wordList componentsNames(ncomps);
    forAll(componentsNames, i)
    {
        componentsNames[i] = components[i];
    }
    componentSolutionIndex_ = (int*)malloc((size_t)(ncomps * sizeof(int)));
    forAll(componentsNames, i)
    {
        for (int j = 0; j < ncomps; ++j)
        {
            std::string tmpComponent = componentsNames[j];
            if (tmpComponent == componentsNames[i])
            {
                componentSolutionIndex_[i] = j;
            }
        }
    }
    if (ncomps > 0) concentration_ = (double*)malloc((size_t)(ncomps * nxyz_ * sizeof(double)));
    // status = phreeqc_.GetSpeciesConcentrations(concentration_);
    forAll(componentsNames, i)
    {
        volScalarField& Yi = Yf_[i];
        forAll(mesh_.cells(), celli)
        {
            c[componentSolutionIndex_[i] * nxyz_ + celli] = Yi[celli] / 1000.;
        }
    }
    // Y_.resize(componentNames.size());
...
//     updatepH();
// }
void Foam::porousReactionsModels::phreeqcRMModels::updateFluidCompositionOnlyReactionsNew()
{
    Info << "Using SNIA (seq. non-iterative) for transport and chemistry coupling." << nl;
    ...
    //     {
    //         // concentration_[componentSolutionIndex_[i] * nxyz_ + celli] = Yi[celli] / 1000;
    //         Yi[celli] = c[componentSolutionIndex_[i] * nxyz_ + celli] * 1000;
    //     }
    // }
